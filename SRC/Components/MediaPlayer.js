...
const MediaPlayer = ({uri, item, userRole, replied, text}) => {
  ...
  console.log('🚀 ~ MediaPlayer ~ modalVisible:', modalVisible);
  ...
  const answerToCustomer = async videoObject => {
    ...
    if (response !== undefined) {
      console.log('🚀 ~ answerToCustomer ~ response:', response?.data);
      //   isLoading
      //     ? setModalVisible(true)
      ...
      <TouchableOpacity
        ...
        <Video
          source={
            ...
            //     ? {uri: item?.video}
            //     : userData?.role == 'customer' && text == 'viewReply'
            //     ? {uri: item?.reply?.reply}
            //     : {uri: item?.video}
          }
          volume={1}
          ref={videoRef}
          ...
          onLoadStart={x => {
            console.log('🚀 ~ VideoReplyScreen ~ x:', x);
            setLoading(true);
          }}
          onLoad={x => {
            console.log('🚀 ~ VideoReplyScreen ~ x:', x);
